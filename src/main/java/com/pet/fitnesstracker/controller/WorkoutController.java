...
public class WorkoutController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<WorkoutResponseDTO> updateWorkout(@PathVariable("id") String workoutId,
        @RequestBody WorkoutRequestDTO workoutRequestDTO) {
        log.info("Updating workout with ID: {}", workoutId);

        return ResponseEntity.ok(service.updateWorkout(workoutId, workoutRequestDTO));
    }
